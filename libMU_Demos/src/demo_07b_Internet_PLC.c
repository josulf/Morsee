/**
 * @addtogroup demos
 * Test de las comunicaciones TCP/IP mediante el uso de un
 * servidor web, consulta a una página de Internet y las comunicaciones
 * con un servidor KepServer
 * @{
 ********************************************************************
...
static char respuesta_form[] =
	...
	"</form>"
	"<form method=POST name=ledMsg action=cmd.html>"
	...
	for(;;) {
		switch( libMU_Internet_GetStatus() ) {
			...
			case NETWORK_IP_OK:
				...
				if( !res ) {
					libMU_Display_DrawString( "Error de conexion", 0, 81, 8 );
				}
				...
				for(;;) {
					...
					/* Siemens TCP/IP info*/
					snprintf( msg, sizeof(msg), "SW:%02X - R%d/W%d", 
						Switches, PLC_Reads, PLC_Writes );
					libMU_Display_DrawString( msg, 0, 56, 15 );
					snprintf( msg, sizeof(msg), "A%d, N%d, B%d / %02X", 
						 tr.address, tr.num_bytes, tr.num_bit, tr_byte );					
					libMU_Display_DrawString( msg, 0, 89, 12 );
					/* Force change in var to check read write info */
					if( libMU_Button_Pressed( BUTTON_SELECT ) ) Switches++;
				}
 ...
 * @param	cmd_param	Parametros del comando GET/POST
